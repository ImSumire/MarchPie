...
const vec3 skyColor = vec3(0.5412, 0.8196, 0.8667);
...
vec3 rotVec3(vec3 v, vec3 axis, float angle) {
  float _cos = cos(angle);
  return v * _cos + cross(axis, v) * sin(angle) + axis * dot(axis, v) * (1.0 - _cos);
}

vec3 render(vec2 uv) {
  vec3 color = skyColor;
  
  vec3 ro = u_camera_pos; // ray origin
  vec3 rd = vec3(uv, u_camera_fov); // ray direction
  rd = rotVec3(rd, vec3(1.0, 0.0, 0.0), u_camera_rot.x);
  rd = rotVec3(rd, vec3(0.0, 1.0, 0.0), u_camera_rot.y);
  // rd = rotVec3(rd, vec3(0.0, 0.0, 1.0), u_camera_rot.z);
